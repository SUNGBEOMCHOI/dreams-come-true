...
class RewardProcessor:
    def __init__(self, reward_condition):
        ...
        self.x0, self.r0 = self.ball_reward[0]
        self.x1, self.r1 = self.ball_reward[1]
        self.done_x0, self.done_r0 = self.done_reward[0]
        self.done_x1, self.done_r1 = self.done_reward[1]
...
class RobotTracker:
    def __init__(self, robot_cfg):
        ...
        # self.parameters.adaptiveThreshConstant = 0.0
        ...
        self.arucoParams.adaptiveThreshWinSizeMin = 50
        self.arucoParams.adaptiveThreshWinSizeMax = 100
        self.arucoParams.adaptiveThreshWinSizeStep = 1
        self.arucoParams.adaptiveThreshConstant = 0
        self.arucoParams.maxMarkerPerimeterRate = 300
        
        # self.mtx, self.dist = None, None
        self.mtx, self.dist = np.array([[563.12798461,0.,337.27099269],[0.,548.2950083,215.06738654],[0.,0.,1.]]),np.array([[-0.81149687,2.91123169,0.01925829,0.01752391,-4.61833363]])
    ...
    def detect_aruco_marker(self, frame):
        ...
        cv2.waitKey(500)
        # corners, ids, rejectedImgPoints = aruco.detectMarkers(gray_frame, self.aruco_dict, parameters=self.parameters)
        ...
        print(corners, ids)
        for rect in rejectedImgPoints:
            rect = rect.astype(int)

            # Reshape the array to the required shape for cv2.polylines()
            rect = rect.reshape((-1, 1, 2))
            cv2.polylines(gray_frame, [rect], True, (0, 255, 0), thickness=5)

        cv2.imshow('frame', gray_frame)
        cv2.waitKey(5000)
        cv2.destroyAllWindows()
        # cv2 image draw rejectedImgPoints
